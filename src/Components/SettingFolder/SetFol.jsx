    ...
    <div className={SetFolCss.SettingFolder__main}>
      ...
      <div className={SetFolCss.main__RightSide}>
        ...
        <div className={SetFolCss.RightSide__CardBlock}>
          {FolderCurPers.some(el => el.focus === true) ? FolderCurPers.filter(el => el.focus === true).map(val => val.card.map(el => <CardBlock key={el.id_card} ChangeInput={ChangeInput} ChangeCheckedCard={ChangeCheckedCard} data={{id_card: el.id_card, front: el.front, back: el.back, time_create: el.time_create}}/>)) : <div />}
        </div>
        <div className={SetFolCss.RightSide__CardSettingBlock}>
          <CardSettingBlock InputValue={InputValue} ChangeInputValue={ChangeInputValue} CheckedCard={CheckedCard} ChangeCheckedCard={ChangeCheckedCard} Folder_Id={FolderCurPers.filter(el => el.focus === true).length > 0 ? FolderCurPers.filter(el => el.focus === true)[0].id_folder : -1}/>
        </div>
      </div>
    </div>
  );
