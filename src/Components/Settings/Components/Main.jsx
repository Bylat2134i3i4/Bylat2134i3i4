...
const Main = () => {
  ...
  const Short_Data = useSelector((state) => state.Autoris.list.short_list);
  ...
  const ChangeIconEvent = (icon) => {
    ...
    dispatch(change_icon({ id: CurrentPerson[0].id, icon: icon }));
  };
  const ChangeLoginEvent = (login) => {
    dispatch(ChangeLogin({ login: login }));
    dispatch(change_login({ id: CurrentPerson[0].id, login: login }));
  };
  const ChangePasswordEvent = (password) => {
    dispatch(ChangePassword({ password: password }));
    dispatch(change_password({ id: CurrentPerson[0].id, password: password }));
  };
  ...
  const CheckPhoneInputs = () => {
    ...
    const isTrue = Short_Data.some((el) => el.login === PhoneInputs.Value2);
    if (PhoneInputs.Value1 !== PhoneInputs.Value2) {
      if (
        !Regular_Expression_For_Phone.test(PhoneInputs.Value1) ||
        !Regular_Expression_For_Phone.test(PhoneInputs.Value2) ||
        !(PhoneInputs.Value1 === CurrentPerson[0].login) ||
        isTrue
      ) {
  ...
  const CheckPassInputs = () => {
    ...
    const isTrue = Short_Data.some((el) => el.password === PassInputs.Value2);
    if (PassInputs.Value1 !== PassInputs.Value2) {
      if (
        !Regular_Expression_For_Pass.test(PassInputs.Value1) ||
        !Regular_Expression_For_Pass.test(PassInputs.Value2) ||
        !(PassInputs.Value1 === CurrentPerson[0].password) ||
        isTrue
      ) {
    ...
    <div className={MainCss.Settig__Main_base}>
      <div className={MainCss.base__TopBlock}>
        ...
        <div className={MainCss.TopBlock__RightSide}>
          ...
          <input
            ...
            className={classNames(MainCss.RightSide__Input, ErrorReactionURL)}
          />
          <button
            className={MainCss.RightSide__Button}
            onClick={() => {
              if (CheckInputValue()) {
      ...
      <div className={MainCss.base__BottonBlock}>
        <div className={MainCss.BottonBlock__Block}>
          ...
          <InputMask
            mask={'+7(\\999)9999999'}
            value={PhoneInputs.Value1}
            onChange={(el) =>
              ChangePhoneInputs({ Value1: el.target.value, Value2: PhoneInputs.Value2 })
            }
            className={classNames(MainCss.BlockLeft__Input, ErrorPhoneInputs.Show1)}
            placeholder='текущий номер'
            alwaysShowMask
          />
          <InputMask
            mask={'+7(\\999)9999999'}
            value={PhoneInputs.Value2}
            onChange={(el) =>
              ChangePhoneInputs({ Value1: PhoneInputs.Value1, Value2: el.target.value })
            }
            className={classNames(MainCss.BlockLeft__Input, ErrorPhoneInputs.Show2)}
            placeholder='новый номер'
            alwaysShowMask
          />
          <button
            className={MainCss.BlockLeft__Button}
            onClick={() => {
              if (CheckPhoneInputs()) {
                setTimeout(() => {
                  ChangeErrProne({ Show1: '', Show2: '' });
                }, 3000);
        ...
        <div className={MainCss.BottonBlock__Block}>
          ...
          <InputMask
            mask={'a9a9a9'}
            value={PassInputs.Value1}
            onChange={(el) =>
              ChangePassInputs({ Value1: el.target.value, Value2: PassInputs.Value2 })
            }
            className={classNames(MainCss.BlockLeft__Input, ErrorPassInputs.Show1)}
            placeholder='текущий пароль'
            alwaysShowMask
          />
          <InputMask
            mask={'a9a9a9'}
            value={PassInputs.Value2}
            onChange={(el) =>
              ChangePassInputs({ Value1: PassInputs.Value1, Value2: el.target.value })
            }
            className={classNames(MainCss.BlockLeft__Input, ErrorPassInputs.Show2)}
